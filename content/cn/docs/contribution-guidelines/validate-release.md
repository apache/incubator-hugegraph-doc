...
如果本地没有 svn 或 gpg 环境, 建议先安装一下
```bash
# 1. 安装svn
ubuntu系统: sudo apt install subversion -y
mac系统: brew install subversion
验证安装是否成功, 执行以下命令:
svn --version

# 2. 安装gpg
ubuntu系统: sudo apt-get install gnupg
mac系统: brew install gnupg
验证安装是否成功, 执行以下命令:
gpg --version

# 3. 下载 hugegraph-svn 目录 (版本号注意填写此次验证版本, 这里以1.0.0为例)
svn co https://dist.apache.org/repos/dist/dev/incubator/hugegraph/1.0.0/
# (注) 如果出现 svn 下载某个文件速度很慢的情况, 可以考虑 wget 单个文件下载, 如下
...
# 2. 信任发版用户 (这里需要信任 3 个, Imba Jin, vaughn, coderzc 依次执行相同操作)
