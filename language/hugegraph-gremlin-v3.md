...
序号   | 语法列表                | 语句详情
----- | ---------------------- | --------------------------
1     | ⽀持业界通⽤的查询语⾔     | 使⽤gremlin语⾔
2     | ⽀持动态查询能⼒          | Hubble组件中可以执⾏任意gremlin语句
3     | ⽀持传参    | Hubble组件中gremlin语⾔执⾏"g.V().has('age', gt(age))"
4     | ⽀持预编译   | Hubble组件中gremlin语⾔多次执⾏"g.V().has('age', gt(age))"
5     | ⽀持顶点的增加   | Hubble组件中gremlin语⾔执⾏ "graph.addVertex(T.label, "person", "name", "marko", "age", 29, "city", "Beijing")"
6     | ⽀持顶点的查询 | Hubble组件中gremlin语⾔执⾏"g.V()"
7     | ⽀持顶点的修改  | Hubble组件中gremlin语⾔执⾏"graph.addVertex(T.label, "person", "name", "marko", "age", 111, "city", "tianJin")"
8     | ⽀持顶点的删除 | Hubble组件中gremlin语⾔执⾏"g.V('1:marko').drop()"
9     | ⽀持边的增加 | Hubble组件中gremlin语⾔<br>1、执⾏ "marko = graph.addVertex(T.label, "person", "name", "marko", "age", 29, "city", "Beijing")"<br>2、执⾏"vadas = graph.addVertex(T.label, "person", "name", "vadas", "age", 27, "city", "Hongkong")"<br>3、执⾏"marko.addEdge("knows", vadas, "date", "20160110")"
10     | ⽀持边的查询    | Hubble组件中gremlin语⾔执⾏"g.E()"
11     | ⽀持边的修改  | Hubble组件中gremlin语⾔<br>1、执⾏ "marko = graph.addVertex(T.label, "person", "name", "marko", "age", 29, "city", "Beijing")"<br>2、执⾏"vadas = graph.addVertex(T.label, "person", "name", "vadas", "age", 27, "city", "Hongkong")"<br>3、执⾏"marko.addEdge("knows", vadas, "date", "20200110")"
12     | ⽀持边的删除   | Hubble组件中gremlin语⾔执⾏"g.E("S1:marko>1>>S1:vadas").drop()"
13     | ⽀持点和边的合并  | Hubble组件中gremlin语⾔<br>1、执⾏ "marko = graph.addVertex(T.label, "person", "name", "marko", "age", 29, "city", "Beijing")"<br>2、执⾏"vadas = graph.addVertex(T.label, "person", "name", "vadas", "age", 27, "city", "Hongkong")"<br>3、执⾏"marko.addEdge("knows", vadas, "date", "20200110")"<br>4、执⾏ "graph.addVertex(T.label, "person", "name", "marko", "age", 29, "city", "BaoDing")"<br>5、执⾏"marko.addEdge("knows", vadas, "date", "20190110")"

- 表达式

序号   | 语法列表                | 语句详情
----- | ---------------------- | --------------------------
14     | ⽀持遍历查询顶点 | Hubble组件中gremlin语⾔执⾏"g.V()"
15     | ⽀持遍历查询边 | Hubble组件中gremlin语⾔执⾏"g.E()"
16     | ⽀持遍历邻接点  | Hubble组件中gremlin语⾔执⾏"g.V().out()"
17     | ⽀持遍历邻接边  | Hubble组件中gremlin语⾔执⾏"g.V('1:josh').out()"
18     | ⽀持多度查询  | Hubble组件中gremlin语⾔执⾏"g.V('1:josh').out('created').in('created').out('knows')"
19     | ⽀持去重  | Hubble组件中gremlin语⾔执⾏"g.V().hasLabel('person').values('age').dedup()"
20     | ⽀持排序  | Hubble组件中gremlin语⾔执⾏"g.V().hasLabel('person').order().by('age')"
21     | ⽀持查询结果限制-limit  | Hubble组件中gremlin语⾔执⾏"g.V().limit(3)"
22     | ⽀持查询结果限制-range  | Hubble组件中gremlin语⾔执⾏"g.V().range(2,4)"
23     | ⽀持查询结果限制-tail  | Hubble组件中gremlin语⾔执⾏"g.V().tail(2)"
24     | ⽀持查询结 果限制-skip  | Hubble组件中gremlin语⾔执⾏"g.V().skip(3)"
25     | ⽀持条件查询-hasLabel  | Hubble组件中gremlin语⾔执⾏"g.V().hasLabel('person')"
26     | ⽀持条件查询-has  | Hubble组件中gremlin语⾔执⾏"g.V().has('person', 'name', 'josh')"<br>注意：   has语句涉及到的属性name，需要提前建⽴⼆级索引
27     | ⽀持条件查询多层-has  | Hubble组件中gremlin语⾔执⾏"g.V().out().has('name','vadas').has('age',27))"<br>注意：   has语句涉及到的属性name，需要提前建⽴⼆级索引；   has语句涉及到的属性age，需要提 前建⽴范围索引；
28     | ⽀持条件查询-hasId  | Hubble组件中gremlin语⾔执⾏"g.V().hasId('1:vadas')"
29     | ⽀持条件查询-hasKey  | Hubble组件中gremlin语⾔执⾏"g.V().properties().hasKey('city')"
30     | ⽀持条件查询-hasValue  | Hubble组件中gremlin语⾔执⾏"g.V().properties().hasValue('Beijing')"
31     | ⽀持过滤查询-filter | Hubble组件中gremlin语⾔执⾏"g.V().filter(label().is('person'))"
32     | ⽀持过滤查询-coin  | Hubble组件中gremlin语⾔执⾏"g.V().coin(0.5)"
33     | ⽀持过滤查询-where  | ubble组件中gremlin语⾔执⾏"g.V('1:marko').as('a').out('created').in('created').where(neq('a'))"
34     | ⽀持-union  | Hubble组件中gremlin执⾏"g.V('1:josh').union(out('created'), both('knows'))"
35     | ⽀持-as  | Hubble组件中gremlin执⾏"g.V().as('a').out('created').as('b').select('a','b')"
36     | ⽀持-select  | Hubble组件中gremlin执⾏"g.V().as('a').out('created').as('b').select('a','b')"
37     | ⽀持统计-count | Hubble组件中gremlin语⾔执⾏"g.V().hasLabel('person').count()"
38     | ⽀持-flod  | Hubble组件中gremlin语⾔执⾏"g.V().out('knows').values('name').fold()"
39     | ⽀持分组-group by | Hubble组件中gremlin语⾔执⾏"g.V().group().by(label).by('name')"
40     | ⽀持判断-is | Hubble组件中gremlin语⾔执⾏"g.V().values('age').is(32)"
41     | ⽀持模式匹配-match  | Hubble组件中gremlin语⾔执⾏"g.V().match(__.as('a').out('created').has('name', 'lop').as('b'),__.as('b').in('created').has('age', 29).as('c'))"
42     | ⽀持定义常量-choose ... constant  | Hubble组件中gremlin语⾔执⾏"g.V().choose(hasLabel('person'),values('name'),constant('inhuman'))"
43     | ⽀持注⼊-inject | Hubble组件中gremlin语⾔执⾏"g.V('1:marko').in('created').values('name').inject('Tom') "
44     | ⽀持分⽀操作-coalesce  | Hubble组件中gremlin语⾔执⾏"g.V('1:marko').coalesce(outE('knows'), outE('created'))"
45     | ⽀持分⽀操作-optional  | Hubble组件中gremlin语⾔执⾏"g.V('1:marko').optional(out('created'))"
46     | ⽀持取样-sample | Hubble组件中gremlin语⾔执⾏"g.V().outE().sample(1)"
47     | ⽀持-aggregate | Hubble组件中gremlin执⾏"g.V().limit(2).aggregate('agg').cap('agg')"

- 运算符和函数

序号   | 语法列表                | 语句详情
----- | ---------------------- | --------------------------
48     | ⽀持函数-sum | Hubble组件中gremlin执⾏"g.V().hasLabel('person').values('age').sum()"
49     | ⽀持函数-max | Hubble组件中gremlin执⾏"g.V().hasLabel('person').values('age').max()"
50     | ⽀持函数-min | Hubble组件中gremlin执⾏"g.V().hasLabel('person').values('age').min()"
51     | ⽀持函数-mean | Hubble组件中gremlin执⾏"g.V().hasLabel('person').values('age').mean()"
52     | ⽀持运算符-math | Hubble组件中gremlin语⾔执⾏"g.V().as('a').out('knows').as('b').math('a + b').by('age')"
53     | ⽀持运算符-not | Hubble组件中gremlin语⾔执⾏"g.V().not(hasLabel('person'))"
54     | ⽀持-多值条件within、without | Hubble组件中gremlin语⾔执⾏<br>1、  "g.V().out().has('name',within('vadas','josh'))"<br>2、  "g.V().out().has('name',without('vadas','josh'))"
55     | ⽀持⽐较运算符lt、lte、gt、gte | Hubble组件中gremlin语⾔执⾏<br>1、  "g.V().out().has('age', lt(30))"<br>2、  "g.V().out().has('age', lte(30))"<br>3、  "g.V().out().has('age', gt(30))"<br>4、  "g.V().out().has('age', gte(30))"<br>5、  "g.V().values('age').is(lt(30))"<br>6、  "g.V().values('age').is(lte(30))"<br>7、  "g.V().values('age').is(gt(30))"<br>8、  "g.V().values('age').is(gte(30))"
56     | ⽀持范围查询inside、between、outside  | Hubble组件中gremlin语⾔执⾏<br>1、  "g.V().out().has('age', inside(30,40))"<br>2、  "g.V().out().has('age', outside(30,40))"<br>3、  "g.V().out().has('age', between(30,40))"<br>4、  "g.V().values('age').is(inside(30, 40))"；<br>5、  "g.V().values('age').is(outside(30, 40))"；<br>6、  "g.V().values('age').is(between(30, 40))"；
57     | ⽀持运算不等于  | Hubble组件中gremlin语⾔执⾏"g.V('1:marko').as('a').out('created').in('created').where(neq('a'))"
58     | ⽀持运算等于  | Hubble组件中gremlin语⾔执⾏"g.V().filter {it.get().label() == 'person'}"
59     | ⽀持逻辑或  | Hubble组件中图展示部分，执⾏"g.V().or(outE('knows'), outE('created')).values('name')"
60     | ⽀持逻辑与  | Hubble组件中gremlin执⾏"g.V().where(outE('created').and().values('age').is(29)).values('name')"
61     | ⽀持逻辑⾮  | Hubble组件中gremlin执⾏"g.V().not(hasLabel('person')).label()"

- 非安全操作

序号   | 语法列表                | 语句详情
----- | ---------------------- | --------------------------
62     | ⽀持检查⽂件读  | Hubble组件中gremlin执⾏"new FileInputStream(FileDescriptor.in)"
63     | ⽀持检查⽂件写  | Hubble组件中gremlin执⾏"new FileOutputStream(new File(\"\"))"
64     | ⽀持检查命令执⾏  | Hubble组件中gremlin执⾏"process=Runtime.getRuntime().exec('cat /etc/passwd');process.waitFor()"
65     | ⽀持检查⽹络攻击  | Hubble组件中gremlin执⾏"new Socket("localhost", 8200)"

- 属性约束

序号   | 语法列表                | 语句详情
----- | ---------------------- | --------------------------
66     | ⽀持属性类型约束  | 执⾏插⼊顶点（错误的属性类型）age属性值字符串graph.addVertex(T.label, "person", "name", "vadas", "age", "26", "city", "Hongkong")
67     | ⽀持属性⾮空约束  | 执⾏插⼊顶点（city属性为空）graph.addVertex(T.label, "person", "name", "vadas", "age", 27, "city", null)
68     | ⽀持索引唯⼀性约束-unqiue  | 1、   Hubble组件中gremlin执⾏"schema.indexLabel("personByAge").onV("person").by("age").unique().ifNotExist().create();"<br>2、重复插⼊相同的age值属性的点

- 超级点索引加速查询

序号   | 语法列表                | 语句详情
----- | ---------------------- | --------------------------
69     | ⽀持通过点内索引查询边  | 1 、⾸先在没有创建索引的情况下查询超级点，记录耗时情况<br>2、然后在有索引的情况下通过索引查询超级点，记录耗时情况(sortKey)<br>举例：查询某⼿机号呼出的⼀周范围内的时间集合<br>g.V('137XXX').outE('call').has('time', between('20200518','20200524'))

- 顶点ID创建

序号   | 语法列表                | 语句详情
----- | ---------------------- | --------------------------
70     | ⽀持⾃动⽣成ID  | 1、利⽤hubble组件进⾏元数据配置操作中，进⾏顶点的创建可以选择⾃动⽣成ID<br>2、添加相同的顶点数据
71     | ⽀持主键ID  | 1、利⽤hubble组件进⾏元数据配置操作中，进⾏顶点的创建可以选择主键ID<br>2、添加重复的顶点数据
72     | ⽀持⾃定义传⼊ID  | 1、利⽤hubble组件进⾏元数据配置操作中，进⾏顶点的创建可以选择⾃定义传⼊ID  （⾃定义字符 串或数字）<br>2、添加重复的顶点数据
...
>>>> secondary index的查询都是基于"是"或者"相等"的查询条件，不⽀持"部分匹配"
>> + Range ⽀持数值类型的范围查询
...
>>>> shard index可以有任意数字或者⽇期属性，但是查询时最多只能提供⼀个范围查找条件，且该范围查找条件的属性
